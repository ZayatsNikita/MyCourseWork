...
namespace PL.Models.ModelsForView
{
    public class BuilderStabdartFilterViewModel
    {
        ...
        {
            var componentList = standarts.Select(x => x.Componet).ToList();
            var serviceList = standarts.Select(x => x.Service).ToList();
            
            componentList.Insert(0, new Componet() { Id = 0, Title = "Все" });
            serviceList.Insert(0, new Service() { Id = 0, Title = "Все" });
